...
const AdminTopTeller = () => {
  ...
  const getData = async (branchCode, month, sort) => {
    ...
    setLoadingData(true);
    setData([]);
    console.log("shopCode: "+branchCode,"shopName: "+defaultShopName,"month:  "+month,"sort: "+sort)

    await getAdminKPIMonthTopTeller(navigation, branchCode, month, sort).then(async(res) => {
      setLoadingData(false);
        ...
        await checkSearchHistory("advancedSearch","AdminKPITopTeller",{"shopCode":branchCode,"shopName":defaultShopName,"month":month,"sort":sort})
    });
  };
  ...
  const checkRole = async () => {
    await _retrieveData("userInfo").then((user) => {
      ...
      _retrieveData("searchData").then((item)=>{
        if(item.key=="advancedSearch"&&item.screenName=="AdminKPITopTeller"){
          console.log(item)
          setBranchCode(item.data.branchCode);
          ...
          setDefaultShopName("Chọn chi nhánh")
        }
      })
    })
  }
